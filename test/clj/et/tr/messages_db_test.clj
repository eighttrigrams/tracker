...
(deftest add-message-test
  (testing "adds message with required fields"
    (let [message (db.message/add-message *ds* *user-id* "John" "Hello" "Test body" nil nil)]
      (is (some? (:id message)))
...
(deftest list-messages-recent-mode-test
  (testing "recent mode returns non-done messages"
    (let [m1 (db.message/add-message *ds* *user-id* "A" "Msg1" "" nil nil)
          _m2 (db.message/add-message *ds* *user-id* "B" "Msg2" "" nil nil)]
...
(deftest set-message-done-test
  (testing "marks message as done"
    (let [message (db.message/add-message *ds* *user-id* "X" "Test" "" nil nil)
          result (db.message/set-message-done *ds* *user-id* (:id message) true)]
      (is (= 1 (:done result)))))

  (testing "can unmark message as done"
    (let [message (db.message/add-message *ds* *user-id* "Y" "Test2" "" nil nil)
...
(deftest delete-message-test
  (testing "deletes message and returns success"
    (let [message (db.message/add-message *ds* *user-id* "Z" "ToDelete" "" nil nil)
...
(deftest message-user-isolation-test
  (testing "users see only their own messages"
    (let [user2 (db.user/create-user *ds* "user2" "pass")
          user2-id (:id user2)]
      (db.message/add-message *ds* *user-id* "AdminSender" "Admin msg" "" nil nil)
      (db.message/add-message *ds* user2-id "User2Sender" "User2 msg" "" nil nil)
...
(deftest delete-user-cleans-up-messages-test
  (testing "deleting user removes their messages"
    (let [user (db.user/create-user *ds* "testuser" "pass")
          user-id (:id user)]
      (db.message/add-message *ds* user-id "Sender" "User msg" "" nil nil)
      (is (= 1 (count (db.message/list-messages *ds* user-id))))
...
(deftest list-messages-sender-filter-test
  (testing "filters messages by sender"
    (db.message/add-message *ds* *user-id* "Alice" "From Alice" "" nil nil)
    (db.message/add-message *ds* *user-id* "Bob" "From Bob" "" nil nil)
    (db.message/add-message *ds* *user-id* "Alice" "Also from Alice" "" nil nil)
...
(deftest list-messages-excluded-senders-test
  (testing "excludes messages by sender"
    (db.message/add-message *ds* *user-id* "Alice" "From Alice" "" nil nil)
    (db.message/add-message *ds* *user-id* "Bob" "From Bob" "" nil nil)
    (db.message/add-message *ds* *user-id* "Charlie" "From Charlie" "" nil nil)
