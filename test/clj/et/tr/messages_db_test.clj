...
(deftest add-message-test
  (testing "adds message with required fields"
    (let [message (db.message/add-message *ds* nil "John" "Hello" "Test body" nil)]
      (is (some? (:id message)))
...
(deftest list-messages-empty-test
  (testing "returns empty list when no messages"
    (is (= [] (db.message/list-messages *ds* nil)))))

(deftest list-messages-recent-mode-test
  (testing "recent mode returns non-done messages"
    (let [m1 (db.message/add-message *ds* nil "A" "Msg1" "" nil)
          _m2 (db.message/add-message *ds* nil "B" "Msg2" "" nil)]
      (db.message/set-message-done *ds* nil (:id m1) true)
      (let [messages (db.message/list-messages *ds* nil {:sort-mode :recent})]
        (is (= 1 (count messages)))
...
(deftest list-messages-done-mode-test
  (testing "done mode returns archived messages"
    (let [m1 (db.message/add-message *ds* nil "A" "Msg1" "" nil)
          _m2 (db.message/add-message *ds* nil "B" "Msg2" "" nil)]
      (db.message/set-message-done *ds* nil (:id m1) true)
      (let [messages (db.message/list-messages *ds* nil {:sort-mode :done})]
        (is (= 1 (count messages)))
        (is (= "Msg1" (:title (first messages))))))))

(deftest set-message-done-test
  (testing "marks message as done"
    (let [message (db.message/add-message *ds* nil "X" "Test" "" nil)
          result (db.message/set-message-done *ds* nil (:id message) true)]
      (is (= 1 (:done result)))))

  (testing "can unmark message as done"
    (let [message (db.message/add-message *ds* nil "Y" "Test2" "" nil)
          _ (db.message/set-message-done *ds* nil (:id message) true)
          result (db.message/set-message-done *ds* nil (:id message) false)]
      (is (= 0 (:done result))))))

(deftest delete-message-test
  (testing "deletes message and returns success"
    (let [message (db.message/add-message *ds* nil "Z" "ToDelete" "" nil)
          result (db.message/delete-message *ds* nil (:id message))]
      (is (= true (:success result)))
      (is (= 0 (count (db.message/list-messages *ds* nil))))))

  (testing "returns nil for non-existent message"
    (let [result (db.message/delete-message *ds* nil 99999)]
      (is (nil? result)))))
...
(deftest message-user-isolation-test
  (testing "users see only their own messages"
    (let [user2 (db.user/create-user *ds* "user2" "pass")
          user2-id (:id user2)]
      (db.message/add-message *ds* nil "AdminSender" "Admin msg" "" nil)
      (db.message/add-message *ds* user2-id "User2Sender" "User2 msg" "" nil)
      (is (= 1 (count (db.message/list-messages *ds* nil))))
      (is (= 1 (count (db.message/list-messages *ds* user2-id))))
      (is (= "Admin msg" (:title (first (db.message/list-messages *ds* nil)))))
...
(deftest list-messages-sender-filter-test
  (testing "filters messages by sender"
    (db.message/add-message *ds* nil "Alice" "From Alice" "" nil)
    (db.message/add-message *ds* nil "Bob" "From Bob" "" nil)
    (db.message/add-message *ds* nil "Alice" "Also from Alice" "" nil)
    (let [filtered (db.message/list-messages *ds* nil {:sender-filter "Alice"})]
      (is (= 2 (count filtered)))
...
(deftest list-messages-excluded-senders-test
  (testing "excludes messages by sender"
    (db.message/add-message *ds* nil "Alice" "From Alice" "" nil)
    (db.message/add-message *ds* nil "Bob" "From Bob" "" nil)
    (db.message/add-message *ds* nil "Charlie" "From Charlie" "" nil)
    (let [filtered (db.message/list-messages *ds* nil {:excluded-senders #{"Alice"}})]
      (is (= 2 (count filtered)))
      (is (not-any? #(= "Alice" (:sender %)) filtered))))

  (testing "excludes multiple senders"
    (let [filtered (db.message/list-messages *ds* nil {:excluded-senders #{"Alice" "Bob"}})]
      (is (= 1 (count filtered)))
