...
(defn update-meet
  ...
  ([ds user-id meet-id fields expected-modified-at]
   (let [set-map (assoc fields :modified_at [:raw "datetime('now')"])
     ...
     (jdbc/execute-one! (db/get-conn ds)
       (sql/format {:update :meets
                    :set set-map
                    :where (db/update-where meet-id user-id expected-modified-at)
...
(defn categorize-meet [ds user-id meet-id category-type category-id]
  ...
  (when (and (meet-owned-by-user? ds meet-id user-id)
    ...
    (let [conn (db/get-conn ds)
      ...
      (jdbc/with-transaction [tx conn]
        (let [applied (db.category-rule/apply-closure! tx :meet_categories :meet_id meet-id closure)]
          (jdbc/execute-one! tx
            (sql/format {:update :meets
