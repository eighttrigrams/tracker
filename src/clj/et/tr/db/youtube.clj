...
(def channel-select-columns [:id :channel_id :name :min_duration_minutes :enabled :scope :importance :added_at])
...
(defn ensure-settings!
  "Create the user's youtube_settings row with defaults if absent. Used so
  adding a channel alone is enough to enroll the user with the worker."
  [ds user-id]
  (let [conn (db/get-conn ds)]
    (jdbc/execute-one! conn
      (sql/format {:insert-into :youtube_settings
  ...
  (when (channel-owned-by-user? ds user-id channel-row-id)
    (let [enabled (:enabled fields)
          set-map (cond-> {}
                    ...
                    (contains? fields :importance) (assoc :importance (:importance fields))
