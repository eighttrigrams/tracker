...
(defn- filtered? [{:keys [search-term importance domain excluded-domains] :as opts}]
  (boolean
...
(defn fetch-resources [app-state auth-headers opts]
  (let [request-id (:fetch-request-id (swap! *resources-page-state update :fetch-request-id inc))
        {:keys [search-term importance context strict sort-mode]} opts
...
(defn fetch-resource-description
  ([app-state auth-headers resource-id]
   (fetch-resource-description app-state auth-headers resource-id nil))
  ([app-state auth-headers resource-id on-loaded]
       ...
       (swap! app-state update :resources
              (fn [resources]
                (mapv #(if (= (:id %) resource-id)
                         (cond-> %
                           (not (contains? % :description)) (assoc :description (:description full))
                           (not (contains? % :tags)) (assoc :tags (:tags full)))
                         %)
                      resources)))
       (when on-loaded (on-loaded full)))
     (fn [resp]
